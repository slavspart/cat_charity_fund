...
from app.api.validators import check_before_edit, check_object_exists, check_name_duplicate, check_invested
...
async def create_new_charity_project(
    ...
    await check_name_duplicate(charity_project.name, CharityProject, charity_project_crud, session)
...
async def partially_update_project(
    ...
    """Только для суперюзеров."""
    project = await check_object_exists(charity_project_id, charity_project_crud, session)
    ...
    if obj_in.name is not None and obj_in.name != project.name:
        await check_name_duplicate(obj_in.name, CharityProject, charity_project_crud, session)
...
async def remove_charity_project(
    ...
    charity_project = await check_object_exists(charity_project_id, charity_project_crud, session)
