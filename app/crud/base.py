...
class CRUDBase:
    ...
    async def get_id_by_name(
        ...
        db_id = await session.execute(select(model.id).where(model.name == name))
        db_id = db_id.scalars().first()
        return db_id

    async def get_by_user(
        self, model: Type[ModelType], session: AsyncSession, user: User
    ):
        objects = await session.execute(select(model).where(model.user_id == user.id))
