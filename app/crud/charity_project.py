...
class CRUDCharityProject(CRUDBase):
    async def invest_donation(
        ...
        unused_donations = await session.execute(
            select(Donation).where(Donation.full_amount > 0).order_by(Donation.create_date))
        ...
        if unused_donations is not None:
            for donation in unused_donations:
                while project.full_amount > project.invested_amount and donation.full_amount > donation.invested_amount:
