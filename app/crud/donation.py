...
class CRUDDonation(CRUDBase):
    async def invest_donation(
        ...
        fnc = await session.execute(
            select(CharityProject).where(CharityProject.full_amount > 0).order_by(CharityProject.create_date))
        fnc = fnc.scalars().all()
        if fnc is not None:
            for project in fnc:
                while donation.full_amount > donation.invested_amount and project.full_amount > project.invested_amount:
                    print('d', donation.invested_amount, 'p', project.invested_amount, project)
